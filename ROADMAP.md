...
| 3 | **Player caps** ✅ *built* | An optional `max_players` on a tournament, enforced when a player registers themselves — through `POST /tournaments/{id}/participants` or a join link. `GET /join/{code}` reports `can_join: false` when full, so an invitation says the event is full rather than offering a button that answers 409. | The cap binds **self-registration only**: it defends against a shared link, not against the organiser's own hand, and they keep the override that lets them edit the field until play starts. Lowering a cap below the current field is refused rather than leaving an event permanently over quota. Fun rounds keep their own fixed ceiling of one group. |
| 4 | **Reminders** ✅ *built* | Two ways in, one path out: the organiser mails the field from the tournament page, and an hourly sweep (`POST /internal/reminders/sweep`) mails events starting within a day that haven't been told. Resend behind a `Mailer` Protocol. | Off unless configured — `NullMailer` when the key or sender is missing, and `conftest.py` installs it for every test so the suite can never send. The sweep is idempotent by construction: it excludes events with an `UPCOMING` reminder already recorded, so a run that fails halfway is safe to repeat. Guarded by a shared secret that, **unset, refuses everything** rather than skipping the check. `scheduled_at` has existed since M2 and this is its first reader. |
| 5 | **Referrals** ✅ *built* | Every player gets a referral code (`MATE-…`) the moment their profile exists; a new player signing up through `?ref=` records `referred_by`; `GET /players/me/referrals` returns your code and how many arrived through it. | **Attribution only, no rewards** — the question worth asking before there is revenue is "is word of mouth working?", and an incentive invented now would collide with a commercial model Phase 3 has yet to build. Attribution happens **once, at creation**: `POST /players` runs on every login, so anything looser would let the last person to send a link claim a player who has been here for months. The code generator is shared with join codes (#2) — one alphabet, two prefixes. |
| 6 | **Per-player stats / history** ✅ *built* | `GET /players/me/stats` — career figures plus every event you've been in, newest first, with your finishing position. `GET /players/me/stats/courses` — your record at each course, hole by hole: times played, average and best strokes, holes won. Fun rounds included. | Finishing positions come from `LeaderboardService.for_tournament`, **not** a SQL `RANK() OVER (…)`: a window function would be a second copy of `rank_leaderboard`'s tie-break living where no Python test reaches, and a player told they came third here and second on the board has been lied to by one of them. Per-hole records group on the hole *number*, not the hole id, so a course whose holes were re-entered doesn't split one hole into two records. `SUM(points)` doubles as holes won, since ADR-007 has no halves. |
